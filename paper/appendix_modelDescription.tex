...
\textbf{Comment} We used the same network setting in the Decomposable Attention paper (cite). That are, network hidden size is $200$; input word embeddings are $200$ embeddings (downsampled from $300$ dimensional GloVe embeddings (cite) by a learnable linear layer); dropout (cite) is $0.2$ for learning; and learning rate is $0.05$.
