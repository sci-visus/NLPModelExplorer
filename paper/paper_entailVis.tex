...
\vspace{-4mm}
 ...
 \vspace{-7mm}
  \caption{
The interface of the NLIZE system. During exploration, we can filter through a large number of sentence pairs summarized in (a) and the current selected pair is displayed in (b). The model internal information (attention) is displayed in (d) and (e). The predicted probability (one of three labels: \emph{neural}, \emph{entailment}, \emph{contradiction}) is shown in the barycentric coordinate in (f). Finally, the high-level model structure and the updates to the model are summarized in (c).
}
% \vspace{-2mm}
\label{fig:teaser}
}
