...
// let rectw = 50;
// let recth = 30;
// let canvas = d3.select('#canvas').append('svg').attr('width', 3000).attr(
// 	'height', 3000);
// let div = d3.select("body").append("div").attr("class", "tooltip").style(
// 	"opacity", 0);
...
function draw_sentence_tree(x, y, node, hOrv, depth, tree_width = 0,
	tree_height = 0) {
	...
	if (typeof(node) == 'string') {
		...
		canvas.selectAll('.treenode').data([node]).enter().append('rect')
			.attr('x', d => {
				return hOrv ? x : tree_width - rectw;
			})
		...
		canvas.selectAll('.treenode_text').data([node]).enter().append('text')
			...
			.attr('x', d => {
				return hOrv ? x + rectw / 2 : tree_width - rectw / 2;
			})
		...
		if (hOrv) {
			...
			if (flag) {
				for (let i = 0; i < node[key].length; i++) {
					sub_t = draw_sentence_tree(x + w, y + recth, node[key][i], hOrv, ++depth,
			...
			//node is filtered
				...
				cur_t.key = rs.rep;
				draw_sentence_tree(x, y + recth, rs.key, hOrv, depth, tree_width,
					tree_height);
			...
			//draw node
			canvas.selectAll('.treenode').data([
					[key, cur_t.key, id]
				]).enter().append('circle')
				.attr('cx', (d, i) => {
					return x + w / 2
			...
			if (flag) {
				for (let i = 0; i < node[key].length; i++) {
					sub_t = draw_sentence_tree(x + rectw, y + h, node[key][i], hOrv, ++depth,
						tree_width, tree_height);
			...
			//node is filtered
				...
				let rs = getRepresentative_leaf(node, hOrv);
				cur_t.key = rs.rep;
				draw_sentence_tree(x + rectw, y, rs.key, hOrv, depth, tree_width,
					tree_height);
...
function draw_dep_collapse_tree(x, y, sen, sen_index, sen_dep_tree, horv) {
	...
	canvas.selectAll('.dep_tree_word').data(sen_index)
		...
		.on('mouseover', function(d, i) {
			if (sen[d] == '.') return;
			d3.selectAll('.' + sen[d] + '_' + d + '_path').style("stroke-dasharray", "")
				.style('stroke', 'orange');
	...
	canvas.selectAll('.dep_tree_dep_path')
		...
		.attr('d', function(d, i) {
			...
			if (horv == 'h') {
				...
				//second point
				data.push({
					'x': text_loc[word1].x * 5 / 6 + text_loc[word2].x * 1 / 6,
					'y': text_loc[word1].y - Math.abs(text_loc[word1].x - text_loc[word2].x) /
						rectw * 15
				});
				//third point
				data.push({
					'x': text_loc[word1].x * 1 / 6 + text_loc[word2].x * 5 / 6,
					'y': text_loc[word1].y - Math.abs(text_loc[word1].x - text_loc[word2].x) /
						rectw * 15
				});
				//third point
				...
				//second point
				data.push({
					'x': text_loc[word1].x - Math.abs(text_loc[word1].y - text_loc[word2].y) /
						recth * 20,
					'y': text_loc[word1].y * 5 / 6 + text_loc[word2].y * 1 / 6
				});
				//third point
				data.push({
					'x': text_loc[word1].x - Math.abs(text_loc[word1].y - text_loc[word2].y) /
						recth * 20,
					'y': text_loc[word1].y * 1 / 6 + text_loc[word2].y * 5 / 6
				});
				//fourth point
				data.push(text_loc[word2]);
			}
	...
	canvas.selectAll('.dep_tree_rel_rect')
		...
		.attr('x', function(d, i) {
			...
			if (horv == 'h') {
				...
				return text_loc[word1].x - Math.abs(text_loc[word1].y - text_loc[word2].y) /
					recth * 20 - rectw / 4
			}
		})
		.attr('y', function(d, i) {
			let word1 = sen[d[0]] + d[0],
				word2 = sen[d[2]] + d[2];
			if (horv == 'h')
				return text_loc[word1].y - Math.abs(text_loc[word1].x - text_loc[word2].x) /
					rectw * 15 - recth / 4;
			else
...
function draw_dep_tree(x, y, sen, sen_dep_tree, horv) {
	...
	canvas.selectAll('.dep_tree_word').data(sen).enter()
		...
		.style('font-size', 12)
		.on('mouseover', function(d, i) {
			d3.selectAll('.' + d + '_' + i + '_path').style("stroke-dasharray", "").style(
				'stroke', 'orange');
			d3.selectAll('.' + d + '_' + i + '_rect').style('stroke', 'orange');
		})
		.on('mouseout', function(d, i) {
			d3.selectAll('.' + d + '_' + i + '_path').style("stroke-dasharray", "4,4").style(
				'stroke', 'steelblue');
...
function Render(para) {
	...
	draw_dep_collapse_tree(x + rectw * 1.5, y, para.target, targ_index, para.target_tree,
		'h');


	//vertical tree
	let source_index = filter(filter_index_v, para.source, para.source_tree);
	draw_dep_collapse_tree(x, y + recth * 1.5, para.source, source_index, para.source_tree,
		'v');


	let col = targ_index.length;
	let row = source_index.length;
	let matrix = filter_matrix(para.matrix, para.target.length, para.source.length,
		targ_index, source_index);
...
function getAggregateIndexs(tree, filter_set, isAggregate, index,
	aggregateindex, depth) {
	...
	if (typeof(tree) == 'string') {
		...
		if (filter_set.indexOf(key + depth) > -1 && !isAggregate) {
			...
			for (let i = 0; i < tree[key].length; i++) {
				getAggregateIndexs(tree[key][i], filter_set, isAggregate, index, indexs,
					depth + 1);
			}
			if (indexs.length > 1)
			...
			for (let i = 0; i < tree[key].length; i++) {
				getAggregateIndexs(tree[key][i], filter_set, isAggregate, index,
					aggregateindex, depth + 1);
			}
		}
	}
}
