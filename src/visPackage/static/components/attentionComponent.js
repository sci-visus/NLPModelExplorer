...
class attentionComponent extends baseComponent {
	...
	&& this.data["currentPair"] !== undefined
	&& this.data['targ_depTree'] !== undefined
	&& this.data['src_depTree'] !== undefined) {
	    ...
	    this.svg.selectAll('.attentionComponent_matrix_rect').data(this.attList)
	    .enter()
	    .append('rect')
	    .attr('class', 'attentionComponent_matrix_rect')
	    .attr('x', (d,i)=>{
		return this.width * 1/4 + (i % this.targWords.length) * rectw;
	    })
	    .attr('y', (d,i)=>{
		return this.height * 1/4 + Math.floor(i / this.targWords.length) * recth;
	    })
	    .attr('width', rectw)
	    .attr('height', recth)
	    .style('stroke', 'black')
	    .style('stroke-width', '1px')
	    .style('fill', d=>{return d3.interpolateRdBu(1-d);});
	    ...
	    .attr("y", (d, i) => this.targPos[i].y)
    	    .attr("transform", (d, i) => {
		return "rotate(-45, "+this.targPos[i].x+' , '+this.targPos[i].y+')';
	    })
            .style("font-size", 10)
    ...
    computeWordPosition(src, targ) {  
		...
		return {'y':(this.height  * 0.75) / this.srcWords.length * (i + 0.5) + this.height / 4, 'x':this.width * 1/4 - this.margin.left};
        });
	
        this.targPos = this.targWords.map((d, i) => {
		return {'x':(this.width  * 0.75) / this.targWords.length * (i + 0.5)+ this.width / 4, 'y':this.height * 1/4 - this.margin.top};
        });
    }
	...
	else if(parseResult.data["sentence"] == this.data["currentPair"][1]){
		this.data['targ_depTree'] = parseResult.data.depTree;
	}
	else{
		throw Error('unknow sentence in attention Component handle parsedSentence');
	}
